...
export const transformArticleToContentItem = (article: any): ContentItem => ({
  ...
  createdAt: (article.createdAt || article.created_at)?.split?.('T')?.[0] || new Date().toISOString().split('T')[0],
  updatedAt: (article.updatedAt || article.updated_at)?.split?.('T')?.[0] || new Date().toISOString().split('T')[0],
